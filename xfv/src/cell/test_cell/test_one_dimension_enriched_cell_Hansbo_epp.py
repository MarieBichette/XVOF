...
class OneDimensionEnrichedHansboCellEPPTest(unittest.TestCase):
    ...
    def test_compute_enriched_elements_new_pressure_with_elasticity(self, mock_add_elasticity,
        ...
        # les valeurs de return permettent juste � la m�thode dene pas planter mais ne sont pas importantes...
        dt = 1
        ...
        mock_add_elasticity.assert_called()
        mock_apply_eos.assert_any_call(self.my_cells, DataContainer().material_target.constitutive_model.eos,
    ...
    def test_compute_enriched_elements_new_part_size(self, mock_compute_size, mock_disc_border_velocity,
                                                     mock_disc_list):
        """
        Test de la m�thode compute_enriched_elements_new_part_size pour OneDimensionEnrichedHansboCell
        """
        Discontinuity.discontinuity_list.return_value = [self.mock_discontinuity]
        vecteur_vitesse_noeuds = np.array([[1., ], [1.5, ]])
        dt = 1.
        u1g = vecteur_vitesse_noeuds[self.mock_discontinuity.mask_in_nodes]
        u2d = vecteur_vitesse_noeuds[self.mock_discontinuity.mask_out_nodes]
        ...
        mock_disc_border_velocity.assert_called_with(self.mock_discontinuity, vecteur_vitesse_noeuds)
        mock_compute_size.assert_called_with(dt, self.mock_discontinuity, u1g, u2d, ug, ud)
    ...
    def test_compute_enriched_element_new_density(self, mock_compute_density, mock_disc_list):
        ...
        mock_compute_density.assert_called_with(self.my_cells.density.current_value,
                                                self.mock_discontinuity.additional_dof_density.current_value,
                                                self.mock_discontinuity)

    @mock.patch.object(Discontinuity, "discontinuity_list", new_callable=mock.PropertyMock)
    @mock.patch.object(OneDimensionCell, "compute_pseudo", spec=classmethod, new_callable=mock.MagicMock)
    def test_compute_enriched_element_new_pseudo(self, mock_compute_pseudo, mock_disc_list):
        ...
        mock_compute_pseudo.assert_any_call(dt, self.mock_discontinuity.additional_dof_density.current_value,
                                            self.mock_discontinuity.additional_dof_density.new_value,
                                            self.mock_discontinuity.right_part_size.new_value,
                                            self.mock_discontinuity.additional_dof_sound_velocity.current_value,
                                            DataContainer().numeric.a_pseudo,
                                            DataContainer().numeric.b_pseudo)
    ...
    def test_compute_enriched_elements_new_time_step(self, mock_compute_dt, mock_disc_list):
        ...
        mock_compute_dt.assert_called_with(DataContainer().numeric.cfl,
                                        ...
                                        self.mock_discontinuity.additional_dof_sound_velocity.new_value,
                                        self.mock_discontinuity.additional_dof_artificial_viscosity.current_value,
                                        self.mock_discontinuity.additional_dof_artificial_viscosity.new_value)
    ...
    def test_compute_enriched_stress_tensor(self, mock_disc_list):
        ...
        self.mock_discontinuity.additional_dof_deviatoric_stress_new = np.array([[2., 0., 3.]])
        self.mock_discontinuity.additional_dof_pressure.new_value = np.array([1.])
        self.mock_discontinuity.additional_dof_artificial_viscosity.new_value = np.array([0.])
    ...
    def test_compute_enriched_deviatoric_strain_rate(self, mock_compute_D, mock_disc_borders, mock_disc_list):
        ...
        self.mock_discontinuity.plastic_cells = True

        self.my_cells.compute_enriched_deviatoric_strain_rate(dt, node_coord_new, node_velocity_new)

        mock_disc_borders.assert_called_with(self.mock_discontinuity, node_velocity_new)
    ...
    @mock.patch.object(OneDimensionHansboEnrichedCell, "compute_enriched_shear_modulus", spec=classmethod,
                       new_callable=mock.MagicMock)
    def test_compute_enriched_deviatoric_stress_tensor(self, mock_compute_G,
        ...
        self.mock_discontinuity.additional_dof_deviatoric_stress_current = np.array([[0., 0., 0.]])
        self.mock_discontinuity.additional_dof_shear_modulus.new_value = np.array([1.])
        self.mock_discontinuity.additional_dof_deviatoric_strain_rate = np.array([[1., -0.5, -0.5]])
        exact_new_S_right = np.array([[2., -1, -1]])

        self.my_cells.compute_enriched_deviatoric_stress_tensor(coord_noeud_new, vitesse_noeud_new, dt)

        mock_compute_G.assert_called_with()
        mock_compute_D.assert_called()
        np.testing.assert_allclose(self.my_cells._deviatoric_stress_new, exact_new_S_left)
        np.testing.assert_allclose(self.mock_discontinuity._additional_dof_deviatoric_stress_new, exact_new_S_right)
    ...
    def test_compute_enriched_shear_modulus(self, mock_disc_list):
        ...
        np.testing.assert_allclose(self.mock_discontinuity.additional_dof_shear_modulus.new_value,
                                   DataContainer().material_target.initial_values.shear_modulus_init)
    ...
    def test_compute_enriched_equivalent_plastic_strain_rate(self, mock_disc_list):
        ...
        self.mock_discontinuity.additional_dof_deviatoric_stress_new = np.array([[4, -2., -2.]])
        self.mock_discontinuity.additional_dof_shear_modulus.current_value = np.array([4.])
        self.mock_discontinuity.additional_dof_yield_stress.current_value = np.array([2.])
        ...
        np.testing.assert_allclose(self.mock_discontinuity._additional_dof_equivalent_plastic_strain_rate,
    ...
    def test_apply_plastic_correction_on_enriched_deviatoric_stress_tensor(self, mock_disc_list):
        ...
        # set partie droite
        self.mock_discontinuity._additional_dof_deviatoric_stress_new = np.array([[5., -2.5, -2.5], ])
        self.mock_discontinuity.additional_dof_yield_stress.current_value = np.array([10.])
        ...
        np.testing.assert_allclose(self.my_cells._deviatoric_stress_new, exact_deviator_plastic, rtol=1.e-5)
        np.testing.assert_allclose(self.mock_discontinuity._additional_dof_deviatoric_stress_new,
    ...
    def test_compute_enriched_yield_stress(self, mock_disc_list):
        ...
        np.testing.assert_allclose(self.mock_discontinuity.additional_dof_yield_stress.new_value,
                                   DataContainer().material_target.initial_values.yield_stress_init)