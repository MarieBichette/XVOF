...
class EnrichElement(RuptureTreatment):
    ...
    def applyTreatment(self, cells, ruptured_cells, nodes, topology, time):
        """
        ...
        if ruptured_cells.any():  # Enrichment is made once for all
            ...
            for cell_tb_enr in np.nonzero(cells_to_be_enr)[0]:
                if Discontinuity.discontinuity_number() < 100:  # � changer pour plusieurs discontinuity
                    if not cells.enriched[cell_tb_enr]:
                        ...
                        # L'initialisation des tailles gauches et droites courantes n'est pas n�cessaire.
                        # On initialise simplement avec des tailles fictives de sorte qu'on peut g�rer le calcul des forces
                        # coh�sives � l'it�ration o� la discontinuti� est cr��e.
                        # Cette taille ficitive permet simplement d'obtenir une ouverture nulle de la fissure �
                        # l'it�ration de cr�ation de la discontinuit�.  Elle sera �cras�e apr�s ce calcul lors de l'appel
                        # de mesh.increment().
                        disc.right_part_size.current_value = \
                            (1. - self.__position_rupture) * cells.size_t[cell_tb_enr]
                        disc.left_part_size.current_value = self.__position_rupture * cells.size_t[cell_tb_enr]
                        # Initialisation de la partie droite des champs pour Hansbo method
                        if DataContainer().material_target.failure_model.type_of_enrichment == "Hansbo" and \
                                not disc.initialisation:
                            cells.initialize_additional_cell_dof(disc)
                            nodes.initialize_additional_node_dof(disc)
                            disc.have_dof_been_initialized()

                        # Ecartement un tout petit peu de la fissure qui vient d'�tre cr��e
