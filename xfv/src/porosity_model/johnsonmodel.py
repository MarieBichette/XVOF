...
class JohnsonModel(PorosityModelBase):  # pylint: disable=too-few-public-methods
    ...
    """
    def __init__(self, initial_porosity_for_johnson, effective_strength_for_johnson, viscosity_for_johnson):
    ...
    def compute_porosity(self, delta_t: float, 
        ...
        condp = (-(pressure)>=peq0)
        condporo = (porosity==1.0)
        condtot = (condp&condporo)
        porosity[condtot] = initial_porosity_for_johnson

        delta_p = np.where(porosity>1.0, (abs(pressure)-effective_strength_for_johnson / 
                                          porosity * np.log(porosity/(porosity-1.0))), 0.)
        delta_p = np.maximum(delta_p,0.)
        porosity_new = self._compute_johnson_porosity(delta_t, pressure, porosity, delta_p)
        return porosity_new
    ...
    def _compute_johnson_porosity(self, delta_t: float, 
        ...
        dalphadt = -((initial_porosity_for_johnson - 1.0)**(2.0/3.0)* porosity*pexp*delta_p)*np.sign(pressure)
